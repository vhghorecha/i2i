...
<<<<<<< HEAD
        ...
        for(id msgDic in messageArray){
            if([[msgDic objectForKey:ID] isEqual:duplicateMsgIdCheck]){
                msgFlag = NO;
            }
        }
...
=======
        ...
        if ([[NSString stringWithFormat:@"%@",[[notification userInfo] objectForKey:@"fromid"]] isEqualToString:[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] objectForKey:LOGGED_IN_USER]]]) {
>>>>>>> issue-663
            
            /* Add message in messageArray and reload data only for message of type 10 i.e standard text messages */
            if (([[NSString stringWithFormat:@"%@",[[notification userInfo] objectForKey:MESSAGE_TYPE_KEY]] isEqualToString:MESSAGE_TYPE_STANDARD] || [[NSString stringWithFormat:@"%@",[[notification userInfo] objectForKey:MESSAGE_TYPE_KEY]] isEqualToString:MESSAGE_TYPE_IMAGE]) && msgFlag) {
            ...
            if([[NSString stringWithFormat:@"%@",[[notification userInfo] objectForKey:MESSAGE_TYPE_KEY]] isEqualToString:MESSAGE_TYPE_VIDEO]){
                NSMutableDictionary *videoMsg = [NSMutableDictionary dictionaryWithDictionary:[notification userInfo]];
                
                [videoMsg setObject:[NSString stringWithFormat:@"Video File : %@",[[notification userInfo] objectForKey:@"message"]]  forKey:@"message"];
                
                [messageArray addObject:videoMsg];
                [chatTable reloadData];
                NSIndexPath *indexPath = [NSIndexPath indexPathForRow:[messageArray count]-1 inSection:0];
                [chatTable scrollToRowAtIndexPath:indexPath atScrollPosition:UITableViewScrollPositionBottom animated:NO];
            }
...
- (void)showOptions {
    ...
    for (NSString *option in @[@"Send Image from Path",@"Send Image-Data",@"Capture Image",@"Capture Video",@"Share Photo From Photo Library",@"Share Video From Photo Library"]) {
